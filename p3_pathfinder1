...
#    detail_points = {initial_position: initial_xy} # Holds the entry point into each cell
    detail_points = {destination: dest_xy}
...
#            new_distance = forward_dist[current_node] + edge_cost
            new_distance = backward_dist[current_node] + edge_cost


            if adjacent_node not in backward_dist or new_distance < backward_dist[adjacent_node]:
                # Assign new distance and update link to previous cell
                backward_dist[adjacent_node] = new_distance
                backward_prev[adjacent_node] = current_node
                # For priority, use distance + heuristic
#                heappush(queue, (new_distance + heuristic (detail_point, dest_xy), adjacent_node))
                heappush(queue, (new_distance + heuristic (detail_point, initial_xy), adjacent_node))
...
'''
            if adjacent_node not in forward_dist or new_distance < forward_dist[adjacent_node]:
                ...
                forward_prev[adjacent_node] = current_node
                # For priority, use distance + heuristic
                heappush(queue, (new_distance + heuristic (detail_point, dest_xy), adjacent_node))
                detail_points[adjacent_node] = detail_point
